...
public interface ConversationsRepository extends JpaRepository<Conversations, Long> {
    ...
    @Query("SELECT new com.talktalk.dto.response.ConversationResponse(c.id, c.avatar, c.title, c.type, c.lastSenderId, c.lastMessage, c.lastMessageAt, cm.lastReadMessageId, cm.unreadCount, cm2.user.id, u.userName, u.avatar) " +
    ...
    List<ConversationResponse> getAllConversation(@Param("userId") Long userId, @Param("title") String title, Pageable pageable);
